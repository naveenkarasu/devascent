export namespace guiapi {
	...
	export class HintInfo {
	    ...
	    walkthroughOwned: boolean;
	    pityStrategyFree: boolean;
	
	    static createFrom(source: any = {}) {
	        return new HintInfo(source);
	    }
	
	    constructor(source: any = {}) {
	        if ('string' === typeof source) source = JSON.parse(source);
	        this.wallet = this.convertValues(source["wallet"], WalletView);
	        this.strategyOwned = source["strategyOwned"];
	        this.walkthroughOwned = source["walkthroughOwned"];
	        this.pityStrategyFree = source["pityStrategyFree"];
	    }
