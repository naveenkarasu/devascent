...
func rustNodeBuildLit(shapeKind string, v any) string {
	...
	if shapeKind == "tree" {
		...
		return "build_tree(vec![" + strings.Join(parts, ", ") + "])"
	}
	parts := make([]string, len(arr))
	for i, e := range arr {
		parts[i] = goScalar(e)
	}
	return "build_list(vec![" + strings.Join(parts, ", ") + "])"
}
...
func rustPrelude(shape Shape) string {
	...
	if shape.Kind == "linkedlist" {
		...
		b.WriteString("fn build_list(a: Vec<i32>) -> Option<Box<ListNode>> { let mut head = None; for &v in a.iter().rev() { let mut n = Box::new(ListNode::new(v)); n.next = head; head = Some(n); } head }\n")
		if shape.RetKind == "node" {
			b.WriteString("fn dump_list(mut n: Option<Box<ListNode>>) -> Vec<i64> { let mut o = Vec::new(); while let Some(node) = n { o.push(node.val as i64); n = node.next; } o }\n")
		}
	}
	if shape.Kind == "tree" {
